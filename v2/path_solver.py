...
def objective(opt_vars,
    ...
    cost = 0
    # collision cost — 5 cm margin (was 0.20 m, a safety hack for the empty-SDF
    # regime). With a real SDF populated by build_sdf.py, 0.20 was conservative
    # to the point of forbidding any path near the table; 0.05 lets the gripper
    # approach objects to grasp them while still penalizing actual collisions.
    if collision_points_centered is not None:
        collision_cost = 0.5 * calculate_collision_cost(poses_homo[start_idx:end_idx], sdf_func, collision_points_centered, 0.05)
...
class PathSolver:
    ...
    def _setup_sdf(self, sdf_voxels):
        # Cell-center grid convention; matches build_sdf.py. See comment in
        # subgoal_solver._setup_sdf for why this differs from upstream ReKep.
