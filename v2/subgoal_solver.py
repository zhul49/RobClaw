...
class SubgoalSolver:
    ...
    def _setup_sdf(self, sdf_voxels):
        # build a callable SDF from LIBERO objects' box collision proxies that approximate the shape
    ...
    def solve(self,
        ...
        # gripper current location (xyz + quat)
        ...
        rot_bounds_min = np.array([-np.pi, -np.pi, -np.pi])
        rot_bounds_max = np.array([np.pi, np.pi, np.pi])
        ...
        if init_pose is not None:
            ip = np.asarray(init_pose, dtype=np.float64)
            init_pose_euler = np.concatenate([ip[:3], T.quat2euler(ip[3:])])
            init_sol = normalize_vars(init_pose_euler, og_bounds)
            from_scratch = True
        elif not from_scratch and self.last_opt_result is not None:
